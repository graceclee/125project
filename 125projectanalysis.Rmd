...
credit_recovery = subset(credit_recovery, select = -c(LEAID, SCHID, COMBOKEY, JJ))
#head/summary
...
#drop irrelevant columns
d_enrollment = subset(d_enrollment, select = c(State_Abbreviation, State, District, School, Dual_Indicator, Male_Total, Female_Total))
```
...
#rename columns
gift_talent = rename(gift_talent, State_Abbreviation = LEA_STATE, District = LEA_NAME, School = SCH_NAME, G_T_Indicator = SCH_GT_IND, Male_Total = TOT_GTENR_M, Female_Total = TOT_GTENR_F)
...
gift_talent = gift_talent[gift_talent$Male_Total != -9,]
gift_talent = gift_talent[gift_talent$Female_Total != -9,]
...
#rename columns
SAT_ACT = rename(SAT_ACT, State_Abbreviation = LEA_STATE, State = LEA_STATE_NAME, District = LEA_NAME, School = SCH_NAME, Male_Total = TOT_SATACT_M, Female_Total = TOT_SATACT_F)

#drop anything that has NA or -9
SAT_ACT = drop_na(SAT_ACT)
SAT_ACT = SAT_ACT[SAT_ACT$Male_Total != -9,]
SAT_ACT = SAT_ACT[SAT_ACT$Female_Total != -9,]
```
...
grad_rate <- grad_rate %>% 
select(AggregateLevel, CountyCode, DistrictCode, SchoolCode, DistrictName, SchoolName, CharterSchool, DASS, ReportingCategory, CohortStudents, Regular.HS.Diploma.Graduates..Count.,Regular.HS.Diploma.Graduates..Rate.) %>% rename(Num_Students = CohortStudents, HS_Diploma_Count = Regular.HS.Diploma.Graduates..Count., HS_Diploma_Rate = Regular.HS.Diploma.Graduates..Rate.)
